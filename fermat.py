...
def fermatNM(n):
    ...
    # stores all the results of combinations that pass tests in the script
    numbersResult = list()
    ...
    for number in maxDenom:
        tempFraction = Fraction(aRecipricol).limit_denominator(
            max_denominator=number)  # prints fraction form of an integer value

        # Ignores if the fraction generated is 1/1
        if tempFraction != Fraction(1, 1):
            tempArray.append(tempFraction)

    # removes duplicate elements and saves the new set to bFractionsArray variable
    bFractionsArray = set(tempArray)
    ...
    if len(bFractionsArray) > 0:
        ...
        firstNumerator = int(str(firstFraction).split('/')[0])
        # Denominator of the first fraction
        firstDenominator = int(str(firstFraction).split('/')[1])

        # Loops through each fraction in the bFractionsArray
        for fract in bFractionsArray:
            # Numerator of the second fraction
            secondNumerator = int(str(fract).split('/')[0])
            # Denominator of the first fraction
            secondDenominator = int(str(fract).split('/')[1])
            ...
            ##
            # The calculations for the left hand side (a^n + b^n)
            lhs = (math.pow(firstNumber, n) + math.pow(secondNumber, n))
            # The calculations for the right hand side (c^n)
            rhs = math.pow(thirdNumber, n)
            ...
            if (1.0 < diffRatio < 1.1):
                ...
                # Add this data to our combinations array stored in numbersResult
                numbersResult.append(data)
...
def start():
    try:
        ...
        print('No value passed for input')
        pprint(e)
        exit()
